...
def print_results(train_metrics, test_metrics):
    print("\n" + "=" * 60)
    ...
    for tag, m in [("Train", train_metrics), ("Test", test_metrics)]:
        print(
            f"{'Linear Regression':<22} {tag:<8} {m['MAE']:>10.2f} {m['RMSE']:>10.2f} {m['R2']:>10.4f} {m['MAPE']:>9.2f}% {m['Acc±10%']:>9.2f}%"
        )
    print()
...
def print_top_coefficients(model, feature_names, top_n=20):
    print("=" * 60)
    print(f"TOP {top_n} FEATURES BY COEFFICIENT (absolute value)")
    print("=" * 60)
    ...
    for rank, idx in enumerate(indices, 1):
        ...
        print(f"  {rank:>3}. {feature_names[idx]:<35} {sign} {abs(coefs[idx]):.4f}")
...
def main():
    ...
    model, train_metrics, test_metrics = train_and_evaluate(X_train, X_test, y_train, y_test)

    print_results(train_metrics, test_metrics)

    print_top_coefficients(model, X.columns.tolist())
